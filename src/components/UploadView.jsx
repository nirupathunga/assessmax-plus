...
export default function UploadView({ clients, onCancel, onSubmit, onSessionExpired }) {
  const [projectName, setProjectName] = useState('');
  const [selectedClientId, setSelectedClientId] = useState('');
  const [floorsCount, setFloorsCount] = useState(1);
  const [selectedDrawingTypeIds, setSelectedDrawingTypeIds] = useState([]); // Empty default selection
  const [errorStatus, setErrorStatus] = useState('');

  // Step wizard state (1 to 10 steps)
  const [currentStep, setCurrentStep] = useState(1);
  const [currentFloorIndex, setCurrentFloorIndex] = useState(1);
  const [stepState, setStepState] = useState('upload');
  ...
  const allUploadedImages = Object.values(moduleImages).flat();
  const totalUploadedCount = allUploadedImages.length;

  const getDynamicQuantities = (stepId) => {
      ...
      <div className="bg-white border border-slate-200/60 rounded-xl p-6 md:p-8 shadow-sm">
        ...
        {currentStep === 1 && (
          <div>
            ...
            <form onSubmit={handleGoToStep2} className="space-y-6">
              {floorsCount > 1 && (
                <div className="bg-emerald-50/70 border border-emerald-200/65 rounded-xl px-4 py-3 flex items-center justify-between text-emerald-800 animate-fade-in shadow-inner">
              ...
              <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3 select-none">
                ...
                >
                  {currentFloorIndex === 1 ? 'Create Project' : 'Next'}
                </button>
              </div>
            </form>
          </div>
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (currentUploadStep) {
                                setModuleImages(prev => ({
                                  ...prev,
                                  [getStepKey(currentUploadStep.id)]: (prev[getStepKey(currentUploadStep.id)] || []).filter(item => item.id !== img.id)
                                }));
                              }
                            }}
                ...
                <table className="w-full text-left border-collapse font-sans">
                  <thead>
                    <tr className="bg-[#4c3e80] text-white">
                      ...
                      <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-right font-semibold">Quantity</th>
                      <th className="py-4 px-6 text-xs font-bold uppercase tracking-wider text-right pr-12 font-semibold">Unit</th>
                    </tr>
                  </thead>
        ...
        {currentStep === transitionStepNumber && (
          <div className="text-center py-10 px-4 font-sans max-w-lg mx-auto animate-fade-in">
            ...
            >
              Configure Floor {currentFloorIndex + 1} <span className="font-semibold text-emerald-100 font-mono">→</span>
            </button>
          </div>
        )}
