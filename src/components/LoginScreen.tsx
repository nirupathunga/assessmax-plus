  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'signin') {
      ...
      setTimeout(() => {
        ...
        const savedCredsRaw = localStorage.getItem('assessmax_user_creds');
        if (!savedCredsRaw) {
        ...
        try {
          ...
          const enteredEmailNormalized = email.trim().toLowerCase();
          const savedEmailNormalized = (savedCreds.email || '').trim().toLowerCase();

          if (enteredEmailNormalized === savedEmailNormalized && password.trim() === (savedCreds.password || '').trim()) {
            onLoginSuccess(enteredEmailNormalized, savedCreds.fullName || 'Workspace Expert');
          } else {
            setErrorMessage('Invalid email or password. Please try again or create an account.');
          }
        } catch (e) {
          setErrorMessage('Error reading registration data. Please register a brand new account.');
        }
      }, 700);
    } else {
      ...
      setTimeout(() => {
        ...
        try {
          localStorage.setItem('assessmax_user_creds', JSON.stringify(userCreds));
          setSuccessMessage('Registration successful! Please sign in with your credentials.');
          setActiveTab('signin');
          setPassword('');
          setConfirmPassword('');
        } catch (err) {
          setErrorMessage('Could not save registration details. Please check custom browser cookies/storage.');
        }
      }, 900);
    }
  };
